      ...
      <main>
        ...
        <section className="relative h-[80vh] min-h-[600px] flex items-end">
          ...
          <div className="relative z-10 container max-w-7xl mx-auto px-6 pb-20 pt-20">
            ...
            <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.7 }} className="font-display text-4xl md:text-6xl lg:text-7xl text-primary-foreground leading-[1.05] mb-6 max-w-3xl">
              {t(`${v}.heroTitle1`)}<br /><span className="italic">{t(`${v}.heroTitle2`)}</span>
            </motion.h1>
            <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="font-body text-primary-foreground/60 text-lg max-w-xl mb-8" dangerouslySetInnerHTML={{ __html: t(`${v}.heroSubtitle`) }} />
          ...
          <div className="container max-w-6xl mx-auto px-6">
            <div className="grid lg:grid-cols-2 gap-20 items-center">
              <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                <p className="text-gold font-body text-xs tracking-[0.3em] uppercase mb-6">{t(`${v}.conceptTag`)}</p>
                <h2 className="font-display text-3xl md:text-5xl text-foreground leading-tight mb-8" dangerouslySetInnerHTML={{ __html: t(`${v}.conceptTitle`) }} />
                <p className="font-body text-muted-foreground leading-relaxed mb-4">{t(`${v}.conceptText1`)}</p>
                <p className="font-body text-muted-foreground leading-relaxed">{t(`${v}.conceptText2`)}</p>
              </motion.div>
        ...
        {/* Features / Qualities with download buttons */}
        <section className="bg-primary py-24 md:py-32">
          <div className="container max-w-6xl mx-auto px-6">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
              <p className="text-gold font-body text-xs tracking-[0.3em] uppercase mb-4">{t(`${v}.featuresTag`)}</p>
              <h2 className="font-display text-3xl md:text-5xl text-primary-foreground mb-6" dangerouslySetInnerHTML={{ __html: t(`${v}.featuresTitle`) }} />
              <div className="flex flex-wrap justify-center gap-4 mt-8">
                <button className="inline-flex items-center gap-2 border border-gold/40 bg-gold/10 px-6 py-3 text-gold font-body text-xs tracking-[0.15em] uppercase hover:bg-gold/20 transition-all duration-300">
                  <Download className="w-4 h-4" /> {t(`${v}.downloadDossier`)}
                </button>
                <button className="inline-flex items-center gap-2 border border-primary-foreground/20 px-6 py-3 text-primary-foreground/70 font-body text-xs tracking-[0.15em] uppercase hover:border-primary-foreground/40 transition-all duration-300">
        ...
        <section className="bg-background py-24 md:py-32">
          <div className="container max-w-7xl mx-auto px-6">
            ...
            <div className="grid md:grid-cols-3 gap-8">
              {types.map((type, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.15 }} className="group border border-border hover:border-gold/30 transition-colors duration-500">
                  ...
                  <div className="p-8">
                    ...
                    <button className="inline-flex items-center gap-2 border border-foreground/20 px-5 py-2.5 text-foreground font-body text-xs tracking-[0.15em] uppercase hover:border-gold hover:text-gold transition-all duration-300">
        ...
        <section className="bg-primary py-24 md:py-32">
          <div className="container max-w-4xl mx-auto px-6 text-center">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
              ...
              <h2 className="font-display text-3xl md:text-5xl text-primary-foreground mb-4" dangerouslySetInnerHTML={{ __html: t(`${v}.ctaTitle`) }} />
              <p className="font-body text-primary-foreground/60 mb-10 max-w-lg mx-auto">{t(`${v}.ctaSubtitle`)}</p>
              <Link to="/contacto" className="inline-flex items-center gap-2 bg-gold/20 border border-gold/40 px-8 py-4 text-primary-foreground font-body text-sm tracking-[0.15em] uppercase hover:bg-gold/30 transition-all duration-500">
                {t(`${v}.requestInfo`)} <ArrowRight className="w-4 h-4" />
              </Link>
            </motion.div>
            <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="mt-12 font-body text-[11px] text-primary-foreground/30 leading-relaxed italic max-w-2xl mx-auto">
              {t(`${v}.disclaimer`)}
            </motion.p>
          </div>
        </section>
